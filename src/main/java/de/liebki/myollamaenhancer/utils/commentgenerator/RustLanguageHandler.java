...
public class RustLanguageHandler implements LanguageHandler {
    @Override
    public boolean isApplicable(final String fileExtension) {
        return FileExtensions.isRustExt(fileExtension);
    }
    ...
    private boolean isPossibleFunctionSignatureStart(final String trimmed, final StringBuilder sigBuffer) {
        // Accept 'fn', 'pub fn', or 'const fn' at any brace depth
        return sigBuffer.isEmpty() && (trimmed.startsWith("fn ") || trimmed.startsWith("pub fn ") || trimmed.startsWith("const fn "));
    }
    ...
    public String insertComment(final String fileContent, final MethodInfo method, final String commentText) {
        return LanguageHandlerUtils.insertRustDocComment(fileContent, method, commentText);
    }
} 