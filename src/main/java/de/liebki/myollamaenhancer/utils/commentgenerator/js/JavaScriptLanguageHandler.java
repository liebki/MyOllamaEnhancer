...
public class JavaScriptLanguageHandler implements LanguageHandler {
    ...
    public boolean isApplicable(final String fileExtension) {
        return FileExtensions.isJavaScriptExt(fileExtension);
    }
    ...
    public List<MethodInfo> findMethods(final String fileContent) {
        ...
        final MethodContext context = new MethodContext();
    ...
    private void updateContext(final String trimmed, final MethodContext context) {
    ...
    private void processLine(final String[] lines, final int currentLine, final String line, final String trimmed,
                             final String fileContent, final MethodContext context, final List<MethodInfo> methods) {
    ...
    private void updateBraceDepthAndOffset(final String line, final MethodContext context) {
    ...
    private boolean isPossibleMethodSignatureStart(final String trimmed, final MethodContext context) {
    ...
    private boolean isArrowFunction(final String line) {
        return (line.startsWith("const ") || line.startsWith("let ") || line.startsWith("var ")) &&
                line.contains("= (") && line.contains("=>");
    }

    private boolean isClassMethod(final String line, final MethodContext context) {
        return context.isInClass() &&
                1 == context.getBraceDepth() &&
                line.matches("^\\w+\\s*\\([^)]*\\)\\s*\\{?$");
    }

    private boolean isObjectMethod(final String line, final MethodContext context) {
    ...
    private boolean shouldAccumulateSignature(final String trimmed, final MethodContext context) {
        if (!context.isSigBufferEmpty()) return true;
        return this.isPossibleMethodSignatureStart(trimmed, context);
    }
